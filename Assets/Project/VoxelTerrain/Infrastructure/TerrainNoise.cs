...
namespace VoxelTerrain
{
    public class MapPreview {
        public enum MapLense { Noise, Climate, Color, ColoredNoise};
        ...
        public MapPreview(int2 size, int chunkWidth, TerrainSettings settings, Biome[] biomes) {
            ...
            Debug.Log(this.size);
            Debug.Log(this.chunkWidth);
            Debug.Log(this.settings);
            Debug.Log(this.biomes);
        }
            ...
            for (int y = 0; y < textureSize.y; y++)
            {
                for (int x = 0; x < textureSize.x; x++)
                {
                    ...
                    for (int i = 0; i < chunkWidth * chunkWidth; i++)
                    {
                        ...
                        Color noiseColor = Color.Lerp(Color.black, Color.white, noiseMap[chunkOffset + i]);

                        values[mapOffet + (i%chunkWidth)] = colorMap[(y * x * chunkWidth) + i] * noiseColor;
                    }
                }
            }
