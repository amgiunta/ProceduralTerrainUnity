...
public class VoxelizedMesh : MonoBehaviour
{
    ...
    void VoxelizeMeshWithGPU()
    {
        ...
        _boundsMin = /*bounds.min;*/ new Vector3(Mathf.Round(bounds.min.x / _size) * _size, Mathf.Round(bounds.min.y / _size) * _size, Mathf.Round(bounds.min.z / _size) * _size);
        ...
        {
            _voxelsBuffer?.Dispose();
            _voxelsBuffer = new ComputeBuffer(xGridSize * yGridSize * zGridSize, 4 * sizeof(float));
        }
    ...
    {
        Profiler.BeginSample("Generate Voxel Mesh");
        var mesh = new Mesh();
        Vector3[] vertices =
        {
            //Front
